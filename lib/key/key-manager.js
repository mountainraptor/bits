  ...
  class KeyManager {
    ...
    addKeyFromFilepath(filepath, {copy=false}={}) {
      ...
      .then((key) => {
        ...
        // If copy=true and the file is already in the keys directory it causes problems
        let copyClobberFree = path.relative(this._keysDir + '/' + path.basename(filepath), filepath);

        if (copy && copyClobberFree) {
